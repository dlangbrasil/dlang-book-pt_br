...
struct Nokta {
    ...
    int uzaklık(Nokta diğerNokta) const {
...
struct Nokta$(HILITE (T)) {
    ...
    $(HILITE T) uzaklık(Nokta diğerNokta) const {
        immutable real xFarkı = x - diğerNokta.x;
        immutable real yFarkı = y - diğerNokta.y;
...
public:
    ...
    $(HILITE T) üstteki() const {
        return elemanlar[$ - 1];
    }

    size_t uzunluk() const {
 ...
 * size_t.max döndürür. */
size_t ikiliAra(const int[] değerler, int değer) {
...
---
size_t ikiliAra$(HILITE (T))(const $(HILITE T)[] değerler, $(HILITE T) değer) {
    // ...
}
---
