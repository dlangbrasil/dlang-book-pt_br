...
struct Nokta {
    ...
    void toString(void delegate(const(char)[]) çıkış) const {
        çıkış.$(HILITE formattedWrite)!"(%s,%s)"(x, y);
    }
}
...
struct Renk {
    ...
    void toString(void delegate(const(char)[]) çıkış) const {
        çıkış.$(HILITE formattedWrite)!"RGB:%s,%s,%s"(r, g, b);
    }
}
...
struct RenkliNokta {
    ...
    void toString(void delegate(const(char)[]) çıkış) const {
        çıkış.$(HILITE formattedWrite)!"{%s;%s}"(renk, nokta);
    }
}
...
struct Poligon {
    ...
    void toString(void delegate(const(char)[]) çıkış) const {
        çıkış.$(HILITE formattedWrite)!"%s"(noktalar);
    }
}
