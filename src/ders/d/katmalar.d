...
$(H6 Birden fazla $(C mixin) parametre değeri)

$(P
$(C mixin) birden fazla parametre değeri aldığında onların $(C string) karşılıklarını otomatik olarak birleştirir:
)
...
$(P
Bu da bazı durumlarda örneğin $(C format) ifadeleri kullanmaktan daha kullanışlı ve okunaklı olabilir:
)
...
$(H6 Dizgi katmalarındaki kod hataları)

$(P
$(IX -mixin, derleyici seçeneği) Üretilen kodun bütünü kaynak kod içinde açıkça görülmediğinden, $(C mixin) ifadelerindeki derleme hatalarının nedenleri kolayca anlaşılmayabilir. $(C dmd)'nin $(C -mixin) komut satırı seçeneği, dizgi katmalarını belirtilen dosyaya yazar ve böylece hata ayıklamada yardımcı olur.
)

$(P
Katmaya çalıştığı kodda bir yazım hatası bulunan aşağıdaki programa bakalım. Ürettiği koddaki yapı üyesinin tanımının sonundaki noktalı virgülün eksik olduğu, $(C mixin) satırına işaret eden derleme hatasından anlaşılamamaktadır:
)

---
string structÜret(string yapıİsmi, string üyeİsmi) {
  import std.format;
  return format!"struct %s {\n  int %s\n}"(yapıİsmi, üyeİsmi);
}

mixin (structÜret("S", "a"));    $(DERLEME_HATASI)
...
$(P
Program $(C -mixin) derleyici seçeneği ile derlendiğinde, derleme hatası, belirtilen $(C mixin_kodlari) dosyasının içindeki bir satıra işaret eder:
)

$(SHELL
$(SHELL_OBSERVED $) dmd $(HILITE -mixin=mixin_kodlari) deneme.d
mixin_kodlari($(HILITE 154)): Error: semicolon expected, not `}`
)

$(P
Standart kütüphane tarafından $(C mixin) ile katılmış olan kodlara ek olarak $(C mixin_kodlari) dosyası içinde aşağıdaki kod da görülecektir:
)
...
struct S {
  int a
}        $(SHELL_NOTE Satır 154)
)


$(P
Dizgi katmalarındaki hataları ayıklama konusunda bir diğer olanak, üretilen kodu derleme sırasında çıktıya yazdıran $(LINK2 /ders/d/pragma.html, $(C pragma(msg)))'dir. $(C mixin) anahtar sözcüğünü geçici olarak $(C pragma(msg)) ile değiştirmeyi gerektirdiğinden bu yöntemin daha az kullanışlı olduğu düşünülebilir:
)
...
$(H5 $(IX sonlandırıcı, mixin) Eklenen sonlandırıcılar)

$(P
Kullanıcı türlerine $(C mixin) ile birden fazla sonlandırıcı işlev eklenebilir. Nesne sonlanırken bu sonlandırıcılar eklendikleri sıranın tersi sırada işletilirler. Bu olanak kullanıcı türlerine birden fazla kaynak eklenmesini ve her kaynağın temizlik kodunun kendisi tarafından getirilmesini sağlar.
)
...
$(H5 $(IX dosya eklemek) $(IX import, dosya) $(IX string import) $(IX import, string) Dosya içeriğinin programa dahil edilmesi)

$(P
Dosyalar derleme zamanında okunabilir ve içerikleri oldukları gibi programa dahil edilebilir. Dosyanın içeriği $(C string) olarak işlem görür ve bir dizginin kullanılabildiği her yerde kullanılabilir. Örneğin, $(C mixin) ile program kodu olarak dahil edilebilir.
)

$(P
Örneğin, $(C dosya_bir) ve $(C dosya_iki) adındaki iki dosyanın içerikleri aşağıdaki gibi olsun.
)
...
$(P
Bu durumda, aşağıdaki programdaki iki $(C import) anahtar sözcüğü, derleme zamanında o dosya içeriklerinden oluşan iki dizgi yerine geçer:
)

---
void main() {
    string s = import ("dosya_bir");
    mixin (import ("dosya_iki"));
}
---

$(P
$(IX -J, derleyici seçeneği) Bu olanak, dosyaların bulundukları klasörlerin $(C -J) derleyici seçeneği ile bildirilmelerini gerektirir. Örneğin, o iki dosya Linux ortamlarında $(C .) ile belirtilen $(I bu klasör) içinde iseler, program aşağıdaki komutla derlenebilir:
)
...
$(P
$(C mixin) ile kullanılan dizginin kod olarak dahil edildiğini de göz önüne alınca programın aşağıdakinin eşdeğeri olduğu görülür:
)
...
---
int[] seç($(HILITE string koşul))(int[] sayılar) {
    int[] sonuç;
