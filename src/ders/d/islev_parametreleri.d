...
$(P
İşlevlerin değer veya yan etki üretebildiklerini görmüştük. $(C in) anahtar sözcüğü parametrenin işlev tarafından yalnızca giriş bilgisi olarak kullanıldığını belirtir. İngilizce'de "içeriye" anlamına gelen "in", parametrenin amacını daha açık ifade eder:
)
...
$(P
$(C const) gibi, $(C in) parametreler de değiştirilemezler:
)
...
int kalanlıBöl(int bölünen, int bölen, $(HILITE out) int kalan) {
...
$(P
$(IX DIP) $(IX -dip1000) Bu anahtar sözcük parametrenin işlev sonlandıktan sonra kullanılmayacağını bildirir. Bu bölümün yazıldığı sırada $(C scope) ancak işlev $(LINK2 /ders/d/islevler_diger.html, $(C @safe)) olarak tanımlanmışsa ve $(C -dip1000) derleyici seçeneği kullanılmışsa etkiliydi. DIP, "D Geliştirme Önerisi" anlamına gelen $(I D Improvement Proposal) teriminin kısaltmasıdır. DIP 1000 bu bölüm yazıldığında henüz deneme aşamasında olduğundan her durumda istendiği gibi çalışmayabilir.
)
...
$(HILITE @safe) int[] işlev($(HILITE scope) int[] parametre) {
    modülDilimi = parametre;    $(DERLEME_HATASI)
    return parametre;           $(DERLEME_HATASI)
}
...
$(P
Yukarıdaki işlev $(C scope) ile verdiği sözü iki yerde bozmaktadır: Onu hem modül kapsamındaki bir dilime atamakta hem de dönüş değeri olarak kullanmaktadır. Bu davranışlar parametrenin işlevin sonlanmasından sonra da kullanılabilmesine neden olacağından derleme hatasına neden olur.
)
