...
$(P
Böyle durumları önlemenin bir yolu, alan bilgisini D'nin $(I nitelik) olanağından yararlanarak sunmaktır. Bu durumda yapıya yeni üye eklenmez; değer, bir işlevin sonucu olarak hesaplanır. İşlevin ismi üye değişken gibi kullanılacak olan isimdir: $(C alan). Bu işlevin dönüş değeri niteliğin değeri haline gelir:
)
...
struct Dikdörtgen {
    ...
    $(HILITE void alan(double yeniAlan)) {
...
public:
    ...
    double $(HILITE en()) const {
        return en_;
    }

    double $(HILITE boy()) const {
        return boy_;
    }
}
---
...
$(P
Nitelik tanımlarında $(C @property) anahtar sözcüğü de kullanılabilir. Ancak, kesinlikle gerekli olmadığından, bu sözcüğün kullanımı önerilmez.
)
...
struct Foo {
    ...
    int b() const {    $(CODE_NOTE @property kullanılmadan tanımlanmış)
        return 42;
    }
}
...
$(P
$(C @property) anahtar sözcüğünün tek etkisi, nitelik söz dizimine sahip farklı çeşitten ifadelerin türlerini belirlerken farkedilir. Aşağıdaki çıktıda görüldüğü gibi, $(C f.a) ve $(C f.b) ifadelerinin türleri faklıdır:
)

$(SHELL
int            $(SHELL_NOTE f.a ifadesinin türü (dönüş değerinin türü))
const int()    $(SHELL_NOTE Foo.b üye işlevinin türü)
)
...
Macros:
        ...
        KEYWORDS=d programlama dili ders dersler öğrenmek tutorial sınıf class sınıflar kullanıcı türleri nitelik property
