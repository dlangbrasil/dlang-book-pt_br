...
struct GününSaati {
// ...
    ref GününSaati opOpAssign(string işleç)(Süre süre)  // (1)
            if (işleç == "+") {                         // (2)
...
$(P
$(C opBinaryRight), işleçlerin her iki tarafında kullanılması beklenen (örneğin $(C int) gibi işleyen) türler tanımlarken kullanışlıdır:
)

---
    auto x = ÖzelTamsayı(42);
    x + 1;    // opBinary!"+" çağrılır
    1 + x;    // opBinaryRight!"+" çağrılır
---

$(P
$(C opBinaryRight)'ın bir başka yaygın kullanımı $(C in) işlecidir çünkü $(C in) işleci genellikle sağ tarafındaki nesne ile ilişkilidir. Bunun bir örneğini aşağıda göreceğiz.
)
...
struct Süre {
    ...
    ref Süre opOpAssign(string işleç)(int miktar)
            if (işleç == "+") {
...
struct Süre {
    ...
    Süre opBinary(string işleç)(Süre sağdaki) const
            if (işleç == "+") {
...
struct GününSaati {
    ...
    bool opEquals(GününSaati sağdaki) const {
...
---
    int opCmp(GününSaati sağdaki) const {
...
$(P
$(IX opCast!bool) $(IX bool, opCast) $(C opCast) açıkça yapılan tür dönüşümleri için olduğu halde, onun $(C bool) özellemesi mantıksal ifadelerde otomatik olarak işletilir:
)
...
$(P
Yine de, $(C opCast)'in $(C bool) özellemesi bütün otomatik $(C bool) dönüşümleri için değildir:
)
...
// ...
    ...
    foo(süre);                $(DERLEME_HATASI)
    bool b = süre;            $(DERLEME_HATASI)
---
...
struct GününSaati {
    ...
    ref GününSaati opOpAssign(string işleç)(Süre süre)
            if (işleç == "+") {
        dakika += süre.dakika;
