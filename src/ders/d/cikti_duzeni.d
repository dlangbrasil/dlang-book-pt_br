...
$(MONO
    %  $(I$(C ayar_karakterleri  genişlik  ayraç  duyarlık  düzen_karakteri))
)
...
$(H5 $(IX $(PERCENT),) $(IX ayraç) $(I ayraç))
...
---
    writefln("%,f", 1234.5678);        // Üçer üçer
    writefln("%,s", 1000000);          // Üçer üçer
    writefln("%,2s", 1000000);         // İkişer ikişer
---
...
$(P
Hane sayısı $(C *) karakteri olarak belirtildiğinde asıl değer bir sonraki parametre değerinden okunur (o parametre $(C int) olmak zorundadır).
)

---
    writefln("%,*s", $(HILITE 1), 1000000);      // Birer birer
---
...
---
    writefln("%,?s", $(HILITE '.'), 1000000);    // Ayraç noktadır
---
...
$(H6 $(IX denetlenen düzen dizgisi) Denetlenen düzen dizgisi)

$(P
$(C format) gibi düzen dizgisi alan bütün işlevlerin ($(C writef), $(C writefln), $(C formattedWrite), $(C readf), $(C formattedRead), vs.) başka bir yazım düzeni daha vardır. Düzen dizgisi bir $(I şablon parametre değeri) olarak belirtilebilir ve böylece düzen dizgisinin doğruluğu derleme zamanında denetlenebilir:
)
...
void main() {
    writefln!"%s %s"(1);       $(DERLEME_HATASI) (fazladan %s)
    writefln!"%s"(1, 2);       $(DERLEME_HATASI) (fazladan 2)
    writefln!"%s %d"(1, 2.5);  $(DERLEME_HATASI) (uyumsuz %d ve 2.5)
}
---

$(P
Yukarıdaki $(C !) karakteri, $(LINK2 /ders/d/sablonlar.html, daha sonraki bir bölümde) göreceğimiz $(I şablon parametre değeri) işlecidir.
)

$(P
($(I Not: Her ne kadar bu yazım düzeni daha güvenli olsa da derleme zamanını uzatabilir.))
)
