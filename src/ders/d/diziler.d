...
$(P
Bu bölüm dizi olanaklarının yalnızca bir bölümünü kapsar. Diğer olanaklarını daha ilerideki $(LINK2 /ders/d/dilimler.html, Başka Dizi Olanakları bölümünde) göreceğiz.
)
...
$(H6 $(IX ~=) $(IX ekleme, dizi) $(IX eleman ekleme, dizi) Dinamik dizilere eleman ekleme)

$(P
$(C ~=) işleci, dinamik dizinin sonuna yeni bir eleman veya yeni bir dizi ekler:
)
...
$(H6 $(IX remove, dizi) $(IX eleman çıkarma, dizi) Dinamik diziden eleman çıkartma)

$(P
Dizi elemanları $(C std.algorithm) modülündeki $(C remove()) işlevi ile çıkartılırlar. Aynı elemanlara erişim sağlayan birden fazla $(I dilim) olabildiğinden $(C remove()) dizideki eleman adedini değiştiremez. Tek yapabildiği, elemanları bir veya daha fazla konum sola kaydırmaktır. Bu yüzden, $(C remove()) işlevinin sonucunun tekrar aynı dizi değişkenine atanması gerekir.
)

$(P
$(C remove())'un iki farklı kullanımı vardır:
)
...
void main() {
    ...
    $(HILITE dizi =) dizi.remove($(HILITE 1));                // Tekrar dizi'ye atanıyor
    writeln(dizi);
}
---
...
$(LI
Çıkartılacak elemanı $(LINK2 /ders/d/kapamalar.html, ilerideki bir bölümde) göreceğimiz bir $(I isimsiz işlev) ile belirtmek. Örneğin, aşağıdaki kod değerleri 42'ye eşit olan elemanları çıkartır.
)
...
void main() {
    ...
    $(HILITE dizi =) dizi.remove!(a => $(HILITE a == 42));    // Tekrar dizi'ye atanıyor
    writeln(dizi);
}
---
...
$(H6 $(IX ~, birleştirme) $(IX birleştirme, dizi) Birleştirme)

$(P
$(C ~) işleci iki diziyi uç uca birleştirerek yeni bir dizi oluşturur. Aynı işlecin atamalı olanı da vardır ($(C ~=)) ve sağdaki diziyi soldaki dizinin sonuna ekler:
)
