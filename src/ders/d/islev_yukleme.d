...
void bilgiVer($(HILITE double) sayı) {
    writeln("Kesirli sayı: ", sayı);
}

void bilgiVer($(HILITE int) sayı) {
    writeln("Tamsayı     : ", sayı);
}

void bilgiVer($(HILITE string) dizgi) {
    writeln("Dizgi       : ", dizgi);
}
...
---
real yediKatı(real değer) {
    return 7 * değer;
}

double yediKatı(double değer) {
    return 7 * değer;
}
...
---
void bilgiVer(File akım, GününSaati zaman) {
