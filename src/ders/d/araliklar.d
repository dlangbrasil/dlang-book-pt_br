...
$(P
Andrei Alexandrescu, $(LINK2 http://ddili.org/makale/eleman_erisimi_uzerine.html, Eleman Erişimi Üzerine) isimli makalesinde aralıkları tanıtır ve aralıkların erişicilerden nasıl daha üstün olabildiklerini gösterir.
)

$(H5 Aralıklar D'de kaçınılmazdır)

$(P
D dilimleri en işlevsel aralık çeşidi olan $(C RandomAccessRange)'e uyarlar ve Phobos, aralıklarla ilgili çok sayıda olanak içerir. Çoğu programda kendi aralık türlerimizi veya aralık işlevlerimizi yazmamız gerekmez. Buna rağmen aralıkların Phobos'ta nasıl kullanıldığını bilmek önemlidir.
)
...
$(H6 $(IX toString, OutputRange) $(C OutputRange) parametreli $(C toString))

$(P
$(C toString) üye işlevleri $(LINK2 /ders/d/kapamalar.html, temsilci parametre alabildikleri gibi), $(C OutputRange) de alabilirler. $(C format), $(C writefln), ve $(C writeln) gibi işlevler de karakterleri bu çıkış aralığının ara belleğine yerleştirerek daha etkin işlerler.
)

$(P
Bütün $(C OutputRange) aralıklarıyla kullanılabilmesi için, böyle tanımlanmış bir $(C toString) işlevinin şablon olarak tanımlanması gerekir. Aşağıdaki örnek bir şablon kısıtlamasından da yararlanıyor:
)
...
$(P
Dikkat ederseniz, $(C main) içindeki kod açıkça $(C OutputRange) tanımlamamaktadır. $(C OutputRange) nesnesini $(C writeln) oluşturur ve yazdıracağı karakterleri önce bu nesnenin ara belleğine yerleştirir:
)
