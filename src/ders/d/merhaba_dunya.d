...
$(P
$(C -run) seçeneğini de kullanışlı bulabilirsiniz. $(C -run), kaynak kodun derlenmesini, programın oluşturulmasını, ve çalıştırılmasını tek komuta indirger. Bu seçenek komut satırındaki son derleyici seçeneği olmalı ve kaynak dosyadan hemen önce belirtilmelidir:
)

$(SHELL
$(DARK_GRAY $) dmd -de -w -unittest $(HILITE -run) merhaba.d
...
$(P
$(IX asm) $(IX body) $(IX __vector) $(IX delete) $(IX typedef) $(IX volatile) $(IX macro) Bir kaç tanesi hariç, bu sözcükleri ilerideki bölümlerde göreceğiz: $(LINK2 http://dlang.org/statement.html#AsmStatement, $(C asm)) ve $(LINK2 http://dlang.org/phobos/core_simd.html#.Vector, $(C __vector)) bu kitabın kapsamı dışında kalıyor; $(C body), $(C delete), $(C typedef), ve $(C volatile) emekliye ayrılmışlardır; ve $(C macro) henüz D tarafından kullanılmamaktadır.
)
