...
struct Deneme {
    ...
    this(char   karakter_parametre = char.init,
         int    tamsayı_parametre  = int.init,
         double kesirli_parametre  = double.init) {
...
struct Deneme {
    ...
    this(char   karakter = char.init,
         int    tamsayı  = int.init,
         double kesirli  = double.init) {
...
$(H6 İlk atama işlemi kurmadır)

$(P
Üyelerin değerleri kurucu içinde belirlenirken her üyeye yapılan ilk atama işlemi özeldir: O işlem, üyenin $(C .init) değerinin üzerine başka bir değer atanması değil, üyenin belirtilen değerle kurulmasıdır. Üyeye yapılan daha sonraki atama işlemleri normal atama olarak işletilir.
)

$(P
Bu özel kuralın nedeni, $(C immutable) ve $(C const) üyelerin çalışma zamanında bilinen değerlerle kurulabilmelerini sağlamaktır. Aksi taktirde, $(C immutable) ve $(C const) değişkenler değiştirilemediklerinden, bu çeşit üyelerin değerlerini çalışma zamanında belirlemek mümkün olmazdı.
)

$(P
Aşağıdaki program atama işleminin $(C immutable) bir üye için nasıl tek kere mümkün olduğunu gösteriyor:
)
...
struct S {
    ...
    this(int m, int i) {
        this.m = m;     $(CODE_NOTE kurma)
        this.m = 42;    $(CODE_NOTE atama (değişebilen üye için mümkün))

        this.i = i;     $(CODE_NOTE kurma)
