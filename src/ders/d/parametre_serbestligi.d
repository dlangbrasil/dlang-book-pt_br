...
---
$(CODE_NAME tabloYazdır)import std.algorithm;

// ...

void tabloYazdır(string başlık,
                 string[string] tablo,
                 string indeksAyracı,
                 string elemanAyracı) {
    writeln("-- ", başlık, " --");
    ...
    // İlk elemandan önce eleman ayracı olmamalı
    if (indeksler.length != 0) {
        auto indeks = indeksler[0];
        write(indeks, indeksAyracı, tablo[indeks]);
        indeksler = indeksler[1..$];    // İlk elemanı çıkart
    }

    // Diğer elemanlardan önce eleman ayracı yazdırılmalı
...
---
$(CODE_XREF tabloYazdır)void main() {
...
void tabloYazdır(string başlık,
                 string[string] tablo,
                 string indeksAyracı $(HILITE = ":"),
                 string elemanAyracı $(HILITE = ", ")) {
    // ...
}
---
...
$(LI $(IX __MODULE__) $(C __MODULE__): Modülün ismi $(C string) türünde)
$(LI $(IX __FILE__) $(C __FILE__): Kaynak dosyanın ismi $(C string) türünde)
$(LI $(IX __FILE_FULL_PATH__) $(C __FILE_FULL_PATH__): Kaynak dosyanın ismi dosya yoluyla birlikte $(C string) türünde)
$(LI $(IX __LINE__) $(C __LINE__): Satırın numarası $(C int) türünde)
$(LI $(IX __FUNCTION__) $(C __FUNCTION__): İşlevin ismi $(C string) türünde)
$(LI $(IX __PRETTY_FUNCTION__) $(C __PRETTY_FUNCTION__): İşlevin tam bildirimi $(C string) türünde)
...
$(LI $(IX __DATE__) $(C __DATE__): Derleme günü $(C string) türünde)

$(LI $(IX __TIME__) $(C __TIME__): Derleme saati $(C string) türünde)

$(LI $(IX __TIMESTAMP__) $(C __TIMESTAMP__): Derleme günü ve saati $(C string) türünde)

$(LI $(IX __VENDOR__) $(C __VENDOR__): Derleyici $(C string) türünde (örneğin, $(STRING "Digital Mars D")))

$(LI $(IX __VERSION__) $(C __VERSION__): Derleyici sürümü $(C long) türünde (örneğin, 2.081 sürümü için $(C 2081L) değeri))
...
---
double topla(double[] sayılar$(HILITE ...)) {
    double sonuç = 0.0;
...
---
char[] parantezle(
        string açma,     // ← işlev çağrılırken belirtilmelidir
        string kapama,   // ← işlev çağrılırken belirtilmelidir
        string[] sözcükler...) {    // ← hiç belirtilmeyebilir
    char[] sonuç;
