...
$(P
$(IX TypeInfo_Class) $(IX .classinfo) $(C TypeInfo)'nun dizi, yapı, sınıf, vs. gibi çeşitli türler için farklı alt sınıfları vardır. Bunlar arasından $(C TypeInfo_Class) özellikle yararlıdır. Örneğin, bir sınıf nesnesinin asıl türü $(C TypeInfo_Class.name) niteliğinden $(C string) olarak öğrenilebilir. Bir sınıf nesnesinin $(C .classinfo) niteliği o nesnenin türüne özgü $(C TypeInfo_Class) nesnesini verir:
)

---
    TypeInfo_Class bilgi = a$(HILITE .classinfo);
    string asılTürİsmi = bilgi$(HILITE .name);
---
...
$(P
Bu işlevin eşleme tablolarında indeks türü olarak kullanılabilmesi için $(C opEquals) işlevinin de tanımlanmış olmaları gerekir.
)
...
$(P
$(C toHash) tanımlandığında $(C opEquals) işlevinin de tanımlanması gerektiğini unutmayın.
)
