...
$(P
$(IX do, sözleşmeli programlama) D'nin sözleşmeli programlama anlayışında işlevlerin giriş koşulları "giriş" anlamına gelen $(C in) bloklarında denetlenir. Sözleşmeli programlama blokları kullanıldığı zaman, işlevin asıl bloğu da $(C do) ile belirlenir:
)
...
$(P
$(IX body) $(I Not: D'nin önceki sürümlerinde bu amaç için $(C do) yerine $(C body) anahtar sözcüğü kullanılırdı.)
)
...
$(LI İşlevin asıl işlemlerini içeren $(C do) bloğu: bu bloğun yazılması şarttır, ama eğer $(C in) ve $(C out) blokları kullanılmamışsa $(C do) anahtar sözcüğü yazılmayabilir)
...
$(H5 $(IX sözleşme ifadeleri) $(IX ifade, sözleşme) Sözleşme ifadeleri)

$(P
$(C in) ve $(C out) bloklarının her çeşit D kodu içerebilmesi doğal olarak yararlı bir olanaktır. Buna rağmen, giriş koşulları ve çıkış garantileri çoğu zaman basit $(C assert) ifadelerinden oluşurlar. Böyle durumlarda bir kolaylık olarak sözleşme ifadelerinden yararlanılabilir. Aşağıdaki işleve bakalım:
)
...
$(P
Sözleşme ifadeleri blok parantezlerini ortadan kaldırır, $(C assert) açıkça çağrılmaz, ve $(C do) anahtar sözcüğüne gerek kalmaz:
)
...
in (b < 10)
out ($(HILITE sonuç;) sonuç > 1000) {
    // ...
}
---

$(P
İşlevin dönüş değerinin $(C out) ifadesindeki noktalı virgülden önce isimlendirildiğine dikkat edin. İşlevin dönüş değeri bulunmadığında veya çıkış garantisi dönüş değeri ile ilgili olmadığında bu noktalı virgül yine de yazılmalıdır:
)
...
$(P
Ben $(C int) seçtiğim halde burada üç değerli bir $(C enum) türü döndürmek daha uygun olurdu:
)
...
$(HILITE MaçSonucu) puanEkle(int goller1, int goller2,
                   ref int puan1, ref int puan2)
// ...
---
