...
$(P
$(I Not:
O bölümdeki "in bloğu mu enforce mu" başlığı altındaki ilkeleri gözetmeniz önemlidir. Bu bölümdeki örnekler nesnelerin ve parametrelerin tutarlılıkları ile ilgili sorunların programcı hatalarına bağlı olduğu durumlarla ilgilidir. Diğer durumlarda ise işlevin kodları içinden $(C enforce)'u çağırmanız doğru olacaktır.)
)
...
$(P
Üçgenin kenarları zaten yapının üye değişkenleri olduklarından bu işlevin parametreleri bulunmuyor. O yüzden bu işlevin $(C in) bloğunu yazmadım. Üye değişkenlerin tutarlılıkları için aşağıdaki bilgileri kullanmanız gerekir.
)
...
$(P
$(C in) blokları üst sınıftan alt sınıfa doğru otomatik olarak işletilir. $(I Herhangi bir) $(C in) bloğunun başarılı olması (bütün $(C assert)'lerin doğru çıkması), giriş koşullarının sağlanmış olduğu anlamına gelir ve işlev çağrısı başarıyla devam eder.
)
