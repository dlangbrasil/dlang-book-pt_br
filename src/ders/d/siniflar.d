...
$(P
$(IX OOP) $(IX NYP) $(IX nesne yönelimli programlama) $(IX kullanıcı türü) Sınıflar kullanıcı türü tanımlamaya yarayan başka bir olanaktır. D'nin nesne yönelimli programlama olanakları sınıflar yoluyla gerçekleştirilir. Nesne yönelimli programlamayı üç temel kavram üzerinde düşünebiliriz:
)
...
$(P
Benzer biçimde, nesnenin $(C immutable) bir kopyası da ismi $(C idup) olan bir işlev tarafından sağlanabilir. Ancak, bu örnekteki kurucu işlevin de $(C pure) olarak tanımlanması gerekir. $(C pure) anahtar sözcüğünü $(LINK2 /ders/d/islevler_diger.html, ilerideki bir bölümde) göreceğiz.
)

---
class Sınıf {
// ...
    this(Yapı yapıNesnesi, const char[] dizgi, int tamsayı) $(HILITE pure) {
        // ...
    }
