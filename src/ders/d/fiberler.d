...
auto düğümleri(const(Ağaç) ağaç) {
    if (ağaç.kök) {
        ...
        return new AralıkTürü(() {});    $(CODE_NOTE Boş aralık)
    }
}
---
...
$(P
O satırın koşulsuz bir döngüdeki bir $(C try-catch) deyimi içine alınması, $(C uint)'e dönüştürülebilecek veri gelene kadar fiberi canlı tutacaktır:
)
