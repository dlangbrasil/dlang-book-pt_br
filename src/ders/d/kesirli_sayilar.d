...
$(LI $(IX .min_normal) $(IX taşma, alttan) $(IX alttan taşma) $(C .min_normal), türün normal duyarlığı ile ifade edebildiği sıfıra en yakın değerdir. (Duyarlık kavramını aşağıda göreceğiz.) Tür aslında daha küçük değerler de ifade edebilir ama o değerlerin duyarlığı türün normal duyarlığının altındadır ve hesaplanmaları daha yavaştır. Bir kesirli sayı değerinin eksi $(C .min_normal) ile artı $(C .min_normal) aralığında olması durumuna (0 hariç) $(I alttan taşma) denir.)
...
$(P
$(I Not: $(C double) ve $(C real) $(C float)'tan daha fazla duyralığa sahip oldukları halde, $(C writeln) bütün kesirli sayı türlerini 6 basamak duyarlıkla yazdırır. (Duyarlık kavramını aşağıda göreceğiz.))
)
...
$(H5 $(IX sırasızlık) Sırasızlık)

$(P
Daha önce tamsayılarda göndüğümüz karşılaştırma işleçleri kesirli sayılarda da kullanılır. Ancak, kesirli sayılarda geçersiz değeri gösteren $(C .nan) da bulunduğundan, o değerin başka değerlerle küçük büyük olarak karşılaştırılması anlamsızdır. Örneğin, $(C .nan)'ın mı yoksa $(C 1)'in mi daha büyük olduğu gibi bir soru yanıtlanamaz.
)

$(P
Bu yüzden kesirli sayılarda başka bir karşılaştırma kavramı daha vardır: sırasızlık. Sırasızlık, değerlerden en az birisinin $(C .nan) olması demektir.)
